...
Sphere::Sphere(glm::vec3 _centre, float _radius) //sphere setup
{
	centre = _centre;
	radius = _radius; 
}
...
glm::vec3 Sphere::shadePixel(Ray _ray, glm::vec3 _inter, Light _light, Camera _camera)
{
	...
	float a = glm::max(glm::dot(lightLocation, _inter),0.0f); //using normal for light shading
	glm::vec3 colour = _light.lightColour * _light.intensity * a; //add it all together
	return colour;
}

glm::vec3 Sphere::shadePixelNormal(Ray _ray, glm::vec3 _inter, Light _light, Camera _camera)
{
	return (0.5f * glm::vec3(_inter.x + 1.0f, _inter.y + 1.0f, _inter.z +1.0f)) * 255.0f; //sets colour in accordance to normals
}
